...
/**
 * Seed all data for all users in the database
 */
  ...
  try {
    ...
    for (const user of users) {
      ...
      if (!wallet) {
        ...
        await wallet.save();
        console.log(`   ✅ Created wallet (balance: 0)`);
      } else {
        console.log(`   ℹ️  Wallet exists (balance: ${wallet.total_balance})`);
      }
      ...
      if (!streak) {
        ...
        await streak.save();
        console.log(`   ✅ Created streak (0 days, 1.0x multiplier)`);
      } else {
        console.log(`   ℹ️  Streak exists (${streak.current_streak_count} days, ${streak.multiplier}x multiplier)`);
      }

      // Clear old attempts and rewards for this user
      await MissionAttempt.deleteMany({ user_id: userId });
      await Reward.deleteMany({ user_id: userId });

      // For demo users (if exists), seed some completed missions
      const demoUser1 = '1c8541c1-dcfa-4ef0-89bc-09b3300d6389';
      const demoUser2 = '820f2141-0b52-460b-8da1-bc37ff695f46';
      
      if (userId === demoUser1) {
        // Update wallet with demo balance
        wallet.total_balance = 2450;
        await wallet.save();

        // Update streak with demo data
        ...
        console.log(`      - Wallet: 2,450 coins`);
        console.log(`      - Streak: 7 days (2.0x multiplier)`);
        console.log(`      - 2 completed missions (Walk 5,000 Steps, Phone-Free 15 min)`);
        console.log(`      - 1 in-progress mission (Track 3 Expenses)`);
        console.log(`      - 2 reward transactions (35 coins total)`);
      } else if (userId === demoUser2) {
        // Update wallet with demo balance for Sneha
        wallet.total_balance = 1850;
        await wallet.save();

        // Update streak with demo data
        streak.current_streak_count = 5;
        streak.max_streak_ever = 5;
        streak.multiplier = 1.5;
        streak.last_completion_date = todayUTC;
        streak.streak_started_at = new Date(todayUTC.getTime() - 4 * 24 * 60 * 60 * 1000);
        streak.badges_earned = [];
        await streak.save();

        // Create mission attempts for Sneha
        const completedMission1 = insertedMissions[0]; // Walk 5,000 Steps
        const completedMission2 = insertedMissions[3]; // Complete Financial Quiz
        const inProgressMission = insertedMissions[4]; // Complete 2 Productivity Tasks
        ...
        const attempt1 = await MissionAttempt.create({
          ...
          mission_id: completedMission1.mission_id,
          verification_status: 'APPROVED',
          completed_at: new Date(now.getTime() - 3 * 60 * 60 * 1000),
          submitted_at: new Date(now.getTime() - 3 * 60 * 60 * 1000),
        });
        ...
        const attempt2 = await MissionAttempt.create({
          ...
          mission_id: completedMission2.mission_id,
          verification_status: 'APPROVED',
          completed_at: new Date(now.getTime() - 1.5 * 60 * 60 * 1000),
          submitted_at: new Date(now.getTime() - 1.5 * 60 * 60 * 1000),
        });
        ...
        const attempt3 = await MissionAttempt.create({
          ...
          mission_id: inProgressMission.mission_id,
          verification_status: 'PENDING',
          submitted_at: new Date(now.getTime() - 45 * 60 * 1000),
        });
        ...
        const reward2 = new Reward({
          ...
          source: 'MISSION_COMPLETION',
          is_credited: true,
          reason: 'Mission completion: Complete Financial Quiz - 25 coins (1.0x multiplier)',
          created_at: new Date(now.getTime() - 1.5 * 60 * 60 * 1000),
        });
        await reward2.save();

        console.log(`   ✅ Created demo data for Sneha:`);
        console.log(`      - Wallet: 1,850 coins`);
        console.log(`      - Streak: 5 days (1.5x multiplier)`);
        console.log(`      - 2 completed missions (Walk 5,000 Steps, Complete Financial Quiz)`);
        console.log(`      - 1 in-progress mission (Complete 2 Productivity Tasks)`);
        console.log(`      - 2 reward transactions (40 coins total)`);
      } else {
        console.log(`   ℹ️  New user - starting with empty wallet and streak`);
      }
    ...
    console.log('\n📊 Summary:');
    console.log(`   - Missions: ${insertedMissions.length} (available for all users)`);
    console.log(`     • 2 EASY missions (15-20 coins)`);
    console.log(`     • 3 MEDIUM missions (25-35 coins)`);
    console.log(`     • 1 HARD mission (50 coins, locked until 3 completed)`);
    console.log(`   - Users processed: ${users.length}`);
    console.log(`   - Wallets: ${users.length}`);
    console.log(`   - Streaks: ${users.length}`);
